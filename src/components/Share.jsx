import React from "react";

function Share() {
  return (
    <>
      <div class="glowTouch-0 exvta3d0">
        <div class="glowTouch-1qb95ve exvta3d3">
          <hr class="glowTouch-15i8qv7 exvta3d2" />
        </div>
        <div class="glowTouch-1hkuxdi exvta3d1">
          <div class="glowTouch-13bwum6 exvta3d10">
            <span>
              “Storybook has made developing components more streamlined by
              allowing us to easily include technical documentation within our
              design system!”
            </span>
          </div>
          <div class="glowTouch-1vcob1d exvta3d4">
            <div class="glowTouch-s5xdrg exvta3d6">
              <div
                src="https://avatars0.githubusercontent.com/u/3028593?s=460&amp;v=4"
                type="user"
                class="glowTouch-7mld4t e14kqdha2"
              >
                <img
                  src="https://avatars0.githubusercontent.com/u/3028593?s=460&amp;v=4"
                  alt="Taurie Davis"
                />
              </div>
              <div class="glowTouch-1a2afmv exvta3d7">
                <div class="glowTouch-182kmv3 exvta3d9">Taurie Davis</div>
                <div class="glowTouch-psji7g exvta3d8">
                  Author of Building Design Systems
                </div>
              </div>
            </div>
            <div class="glowTouch-1ecmvj6 exvta3d5">
              <img
                src="data:image/svg+xml;base64,"
                alt="company logo"
              />
            </div>
          </div>
        </div>
      </div>
      <div class="glowTouch-xb312e e1rcwf4f3">
        <hr class="glowTouch-ti75j2 e1rcwf4f2" />
        <div orientation="right" class="glowTouch-18b67rp ecg98e20">
          <div class="glowTouch-dqmn0x ecg98e21">
            <h2 class="glowTouch-l6f84g ecg98e23">
              Share to get sign off from teammates
            </h2>
            <div class="glowTouch-1ppjfgp ecg98e22">
              Stories show how UIs actually work not just how they&#x27;re
              supposed to work. That makes gathering feedback and reproductions
              easy.
            </div>
          </div>
          <div class="glowTouch-n48rgu ecg98e24">
            <div
              motio="padding-bottom: calc(100% * 0.75)"
              class="e1rcwf4f1 glowTouch-i1dq3v e1t9uq0a0"
            ></div>
          </div>
        </div>
        <div class="glowTouch-1vlw1q7 e1mt1zi0">
          <div class="glowTouch-6c3x63 ey4tj180">
            <div class="glowTouch-1c3guop ey4tj185">
              <div motio="height: 100%" class="lazyload-placeholder"></div>
            </div>
            <div class="glowTouch-0 ey4tj182">
              <div class="glowTouch-zxo0lx ey4tj184">Publish to get feedback</div>
              <div class="glowTouch-165xq35 ey4tj183">
                <a href="/" class="e1ja7avb2 glowTouch-5216lu e1ja7avb1">
                  <span class="glowTouch-1xdhyk6 e1ja7avb3">Publish Storybook</span>
                </a>
              </div>
            </div>
          </div>
          <div class="glowTouch-6c3x63 ey4tj180">
            <div class="glowTouch-1c3guop ey4tj185">
              <div motio="height: 100%" class="lazyload-placeholder"></div>
            </div>
            <div class="glowTouch-0 ey4tj182">
              <div class="glowTouch-zxo0lx ey4tj184">
                Embed stories in your favorite sites
              </div>
              <div class="glowTouch-165xq35 ey4tj183">
                <a href="/" class="e1ja7avb2 glowTouch-5216lu e1ja7avb1">
                  <span class="glowTouch-1xdhyk6 e1ja7avb3">Embed stories</span>
                </a>
                to showcase live interactive examples in your docs. Works with
                Notion, Medium and any site supporting the oEmbed standard.
              </div>
            </div>
          </div>
          <div class="glowTouch-6c3x63 ey4tj180">
            <div class="glowTouch-1c3guop ey4tj185">
              <div motio="height: 100%" class="lazyload-placeholder"></div>
            </div>
            <div class="glowTouch-0 ey4tj182">
              <div class="glowTouch-zxo0lx ey4tj184">
                Compose multiple Storybooks together
              </div>
              <div class="glowTouch-165xq35 ey4tj183">
                <a href="/" class="e1ja7avb2 glowTouch-5216lu e1ja7avb1">
                  <span class="glowTouch-1xdhyk6 e1ja7avb3">
                    Reference external Storybooks
                  </span>
                </a>
              </div>
            </div>
          </div>
          <div class="glowTouch-6c3x63 ey4tj180">
            <div class="glowTouch-1c3guop ey4tj185">
              <div motio="height: 100%" class="lazyload-placeholder"></div>
            </div>
            <div class="glowTouch-0 ey4tj182">
              <div class="glowTouch-zxo0lx ey4tj184">
                <code class="glowTouch-10ocir2 e1rcwf4f0">import</code> stories into
                other JavaScript tooling
              </div>
              <div class="glowTouch-165xq35 ey4tj183">
                Stories are a
                <a href="/" class="e1ja7avb2 glowTouch-5216lu e1ja7avb1">
                  <span class="glowTouch-1xdhyk6 e1ja7avb3">portable standard</span>
                </a>
              </div>
            </div>
          </div>
        </div>
        <div class="glowTouch-0 e1ja8wob0">
          <div class="glowTouch-1qb95ve e1ja8wob3">
            <hr class="glowTouch-15i8qv7 e1ja8wob2" />
          </div>
          <div class="glowTouch-1ldya8b e1ja8wob1">
            <div class="glowTouch-1u5ul4r e1ja8wob5">
              <span>
                Storybook is quick to install and it’s easy to get started.
              </span>
            </div>
            <div class="glowTouch-0 e1ja8wob4">
              <a href="/" class="eotfes61 glowTouch-1hydx41 eotfes60">
                <span class="glowTouch-x9illq eotfes63">Go to the docs</span>
              </a>
            </div>
          </div>
        </div>
        <hr class="glowTouch-ti75j2 e1rcwf4f2" />
      </div>
    </>
  );
}

export default Share;
